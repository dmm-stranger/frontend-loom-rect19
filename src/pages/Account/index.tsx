  ...
  return (
    <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto', padding: '32px 28px' }}>
      ...
      <div style={{ display: 'flex', gap: 28, alignItems: 'flex-start' }}>
        ...
        {/* Sidebar */}
        <aside style={{ width: 220, flexShrink: 0, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: 8, position: 'sticky', top: 'calc(var(--header-h) + 16px)', alignSelf: 'flex-start' }}>
...
function OrdersSection() {
  ...
  if (isLoading) return <div style={{ display: 'flex', justifyContent: 'center', padding: '60px 0' }}><Spinner size={36} /></div>

  return (
    <div>
      <h2 style={{ fontFamily: 'var(--font-sans)', fontWeight: 700, fontSize: 18, color: 'var(--text)', marginBottom: 20 }}>
        My Orders
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--text-muted)', marginLeft: 10, fontWeight: 400 }}>({orders.length})</span>
      </h2>
        ...
        {orders.map((order: any) => (
          <div key={order._id} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10, marginBottom: 16 }}>
              <div>
                <p style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-muted)', marginBottom: 4 }}>ORDER #{order._id.slice(-8).toUpperCase()}</p>
                <p style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-muted)' }}>
                  {new Date(order.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
                </p>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', padding: '4px 10px', borderRadius: 4, color: STATUS_COLORS[ order.orderStatus ] || 'var(--text-muted)', background: `${STATUS_COLORS[ order.orderStatus ]}18`, border: `1px solid ${STATUS_COLORS[ order.orderStatus ]}33` }}>
                  {order.orderStatus}
                </span>
            ...
            <div style={{ borderTop: '1px solid var(--border)', marginTop: 16, paddingTop: 14, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
              <div style={{ display: 'flex', gap: 16 }}>
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-muted)' }}>Subtotal: {formatCurrency(order.itemsPrice)}</span>
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-muted)' }}>Tax: {formatCurrency(order.taxPrice)}</span>
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-muted)' }}>Shipping: {order.shippingPrice === 0 ? 'FREE' : formatCurrency(order.shippingPrice)}</span>
              </div>
              <span style={{ fontFamily: 'var(--font-sans)', fontWeight: 700, fontSize: 15, color: 'var(--text)' }}>Total: {formatCurrency(order.totalPrice)}</span>
            </div>
          </div>
        ))}
      </div>
...
function WishlistSection() {
  ...
  return (
    <div>
      <h2 style={{ fontFamily: 'var(--font-sans)', fontWeight: 700, fontSize: 18, color: 'var(--text)', marginBottom: 20 }}>
        Wishlist
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--text-muted)', marginLeft: 10, fontWeight: 400 }}>({wishlistProducts.length})</span>
      </h2>
          ...
          {wishlistProducts.map((p: any) => (
            <Link key={p._id} to={`/products/${p.slug}`} style={{ textDecoration: 'none' }}>
              <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                <p style={{ fontFamily: 'var(--font-sans)', fontSize: 15, fontWeight: 700, color: 'var(--text)' }}>
                  {formatCurrency(p.discountPrice > 0 ? p.discountPrice : p.price)}
                </p>
              </div>
            </Link>
          ))}
